...
import com.ferra13671.BThack.impl.Commands.PacketList.AbstractPacketListCommand;
import com.ferra13671.BThack.impl.Commands.PacketList.EditPacketListCommand;
...
public class PacketList {
    ...
    public final EditPacketListCommand editPacketListCommand;
    public final AbstractPacketListCommand packetListCommand;

    public final ArrayList<Class<? extends Packet>> packets = new ArrayList<>();
    public final ArrayList<String> packetNames = new ArrayList<>();

    public PacketList(String descName, String alias, String txtName) {
        editPacketListCommand = new EditPacketListCommand(descName, alias, this);
        packetListCommand = new AbstractPacketListCommand(descName, alias + "list") {
            @Override
    ...
    public void saveInFile() throws IOException {
        ConfigUtils.saveInTxt(txtName, editPacketListCommand.descName, writer -> {
    ...
    public void loadFromFile() throws IOException {
        ConfigUtils.loadFromTxt(txtName, editPacketListCommand.descName, line -> {
            if (PACKETS.containsKey(line)) {
                packets.add(PACKETS.get(line));
    ...
    public void addToList(String packet) {
        if (!packetNames.contains(packet)) {
            packets.add(PACKETS.get(packet));
            packetNames.add(packet);
            try {
                saveInFile();
            } catch (IOException ignored) {}
    ...
    public void removeFromList(String packet) {
        if (packetNames.contains(packet)) {
            packets.remove(PACKETS.get(packet));
    ...
    public void clearList() {
        ...
        ChatUtils.sendMessage(Formatting.AQUA + String.format(LanguageSystem.translate("lang.command.List.listCleared"), editPacketListCommand.descName));
    }
