...
public class BreakCommand extends AbstractCommand {
    ...
    public void compile(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(arg("x delta", Arguments.DOUBLE).then(arg("y delta", Arguments.DOUBLE).then(arg("z delta", Arguments.DOUBLE).executes(context -> {
            ...
            thread3D.set3DSchematic(new ArrayList<>(Arrays.asList(new Vec3d(context.getArgument("x delta", Double.class), context.getArgument("y delta", Double.class), context.getArgument("z delta", Double.class)))), new ModifyBlockPos(mc.player.getX(), mc.player.getY(), mc.player.getZ()));
            thread3D.start();
            return SUCCESFUL;
        }))));
    }
